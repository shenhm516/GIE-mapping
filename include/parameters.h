...
//    display option
    ...
    void setupParam(const ros::NodeHandle &nh)
    {
        ...
        std::vector<double> extrinsic_T, external_T;
        nh.param<std::vector<double>>("GIE_mapping/extrinsic_T", extrinsic_T, std::vector<double>());
        nh.param<std::vector<double>>("GIE_mapping/external_T", external_T, std::vector<double>());
        odom_T_L<<MAT_FROM_ARRAY(extrinsic_T);
        W_T_W<<MAT_FROM_ARRAY(external_T);
        std::cout << odom_T_L << std::endl;
        std::cout << W_T_W << std::endl;
    }
