                             ...
                             int ext_obs_num, bool* obs_activated, float3* obsbbx_ll, float3* obsbbx_ur)
{
    ...
    for (loc_crd.x = 0; loc_crd.x < loc_map._local_size.x; ++loc_crd.x) {
        ...
        char occ_type = loc_map.get_vox_type(loc_crd);
        ...
        int3 vb_key = get_VB_key(glb_crd); //shm: hash key in global frame
        ...
        // update observed occ val from sensor/ext observ (free or occ)
        
        // if (count > 0/* || occ_flag*/)
        if (occ_type==VOXTYPE_OCCUPIED)
        {
            // set_hashvoxel_occ_val(cur_vox,250.f, 1.f, loc_map._occu_thresh, time);
            if (old_glb_type!=VOXTYPE_FREE)
                cur_vox->vox_type = VOXTYPE_OCCUPIED;
            else cur_vox->vox_type = VOXTYPE_FREE;
        }
        // else if (count < 0 && old_glb_type!=VOXTYPE_OCCUPIED)
        else if (occ_type == VOXTYPE_FREE && old_glb_type!=VOXTYPE_OCCUPIED)
        {
            // float pbty = min(1.f,static_cast<float>(-count)/10.f);
            // set_hashvoxel_occ_val(cur_vox,0.f,pbty,loc_map._occu_thresh, time);
            cur_vox->vox_type = VOXTYPE_FREE;
            // loc_map.set_vox_glb_type(loc_crd, VOXTYPE_FREE);
        }// else loc_map.set_vox_glb_type(loc_crd, cur_vox->vox_type);
                             ...
                             int ext_obs_num, bool* obs_activated, float3* obsbbx_ll, float3* obsbbx_ur)
{
    ...
    for (loc_crd.x = 0; loc_crd.x < loc_map._local_size.x; ++loc_crd.x) {
        ...
        char new_vox_type = loc_map.get_vox_type(loc_crd); //shm: vox type in local map
        // reset observation of one scan
        loc_map.set_vox_type(loc_crd, VOXTYPE_UNKNOWN);
        ...
        char old_glb_type = cur_vox->vox_type; //shm: vox type in global map
        // update observed occ val

        if (new_vox_type == VOXTYPE_OCCUPIED/* || occ_flag && old_glb_type!=VOXTYPE_FREE*/) {
            // set_hashvoxel_occ_val(cur_vox,250.f, 0.8f, loc_map._occu_thresh, time);
            cur_vox->vox_type = VOXTYPE_OCCUPIED;
        } else if (new_vox_type == VOXTYPE_FREE && old_glb_type!=VOXTYPE_OCCUPIED) {
