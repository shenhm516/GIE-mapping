...
void Vlp16MapMaker::updateLocalOGM(const Projection &proj, const sensor_msgs::PointCloud2ConstPtr& pyntcld,
    ...
    // pcl::fromROSMsg(*pyntcld, pl_orig);
    // sensor_msgs::PointCloud2 laserCloudmsg;
    // pcl::toROSMsg(pl_orig, laserCloudmsg);
    ...
    // *_pntcld_ptr = laserCloudmsg;
    // convert point cloud to rings
    convertPyntCld(pyntcld, _pcl_in_pub, _pcl_out_pub);
    // copy 3d scan
    for(int j=0;j<_mul_scan_param.ring_num;j++)
    {
...
void Vlp16MapMaker::convertPyntCld(const sensor_msgs::PointCloud2ConstPtr &msg, ros::Publisher _pcl_in_pub, ros::Publisher _pcl_out_pub) {
    ...
    pcl::PointCloud<pcl::PointXYZ> laserCloudBody;
    for (int i=0; i<pl_orig.size(); i++) {
        const float x = pl_orig.points[i].x;  // x
        const float y = pl_orig.points[i].y;  // y
        const float z = pl_orig.points[i].z;  // z
        const int bin = (atan2f(y, x) + static_cast<float>(M_PI)) / RESOLUTION;

        float range_hor = sqrtf(x * x + y * y);
        float phi = atan2(z, range_hor);
        int r = floor((phi - _mul_scan_param.phi_min)/_mul_scan_param.phi_inc + 0.5f);
        if ((bin >= 0) && (bin < static_cast<int>(SIZE)) && r>=0 && r<_mul_scan_param.ring_num) {
            scanlines[r].ranges[bin] = range_hor;
            // scanlines[r].intensities[bin] = i;
            laserCloudBody.points.push_back(pl_orig.points[i]);
        }
    }
    sensor_msgs::PointCloud2 laserCloudmsg;
    ...
    _pcl_out_pub.publish(laserCloudmsg);
    // _pcl_in_pub.publish(*msg);
}
