...
VOLMAPNODE::VOLMAPNODE()
{
    ...
    if(param.data_case == "ugv_corridor" ) {
        ...
        pntcld_sync = new message_filters::Synchronizer<pntcld_sync_policy> (pntcld_sync_policy(10), s_pntcld_sub, s_odom_sub);
        pntcld_sync->setMaxIntervalDuration(ros::Duration(0.001));
        ...
        s_laser_sub.subscribe(_nh,"/ouster/scan", 1);
        ...
        s_odom_sub.subscribe(_nh,"/Odometry", 1);
        s_pntcld_sub.subscribe(_nh,"/cloud_registered_body", 1);
        pntcld_sync = new message_filters::Synchronizer<pntcld_sync_policy> (pntcld_sync_policy(1), s_pntcld_sub, s_odom_sub);
    ...
    // std::cout << "Hello 1" << std::endl;
    _pcl_in_pub = _nh.advertise<PntCld>("vlp_16_in", 1);
    _pcl_out_pub = _nh.advertise<PntCld>("vlp_16_out", 1);
    _dbg_rviz_pub =  _nh.advertise<PntCldI>("dbg_pt", 1);
    _dbg_pnt_cld = PntCldI ::Ptr(new PntCldI);
    _dbg_pnt_cld->header.frame_id = "map";
    // std::cout << "Hello 2 " << param.for_motion_planner << std::endl;
    ...
    // Timer
    _mapTimer = _nh.createTimer(ros::Duration(0.01), &VOLMAPNODE::publishMap, this);
...
//---
Projection proj_prev;
void VOLMAPNODE::publishMap(const ros::TimerEvent&)

// void VOLMAPNODE::publishMap()
{
    // std::cout << _msg_mgr.is_ready() << std::endl;
    if(!_msg_mgr.is_ready()) return;
    ...
    else if(_msg_mgr.got_pnt_cld)
    {
        if(param.data_case == "laser3D") {
            _vlp_map_maker.updateLocalOGM(proj, _pntcld_ptr, thrust::raw_pointer_cast(_hash_map->VB_keys_loc_D.data()), _time,
                                          param.for_motion_planner, param.robot_r2_grids, _pcl_in_pub, _pcl_out_pub); //shm: update local OGM
        } else {
            _pnt_map_maker.updateLocalOGM(proj,proj_prev,_pntcld_ptr, thrust::raw_pointer_cast(_hash_map->VB_keys_loc_D.data()), _time,
                                          param.for_motion_planner, param.robot_r2_grids, _pcl_in_pub);
        }
    }

    update_ext_map();
    // std::cout << "got_pnt_cld: " << _msg_mgr.got_pnt_cld << std::endl; 
    _hash_map->updateHashOGM(_msg_mgr.got_pnt_cld  && param.data_case != "laser3D",
                             _time, param.display_glb_ogm && !param.display_glb_edt,
                             ext_obs); //shm: update the Global OGM according to the local OGM infomation
    ...
    if(vis_cnt%param.vis_interval ==0) visualize(proj.origin);
    vis_cnt++;
...
tf::Transform VOLMAPNODE::odom2trans()  {
    ...
    transEigen.matrix() = param.W_T_W*transMat*param.odom_T_L;
    tf::transformEigenToTF(transEigen, trans);
...
{
    // printf("%.6f %.6f\n", pcld->header.stamp.toSec(), odom->header.stamp.toSec());
    CB_odom(odom);
    CB_pntcld(pcld);
    // publishMap();
}
...
void VOLMAPNODE::CB_pntcld(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    if(param.data_case=="laser3D") {
        if(!_vlp_map_maker.is_initialized()) {
            MulScanParam mp(440, 16, 2.0 * M_PI/440, -M_PI, 2.0f/180.0f *M_PI, -15.0f/180.0f*M_PI);
...
void VOLMAPNODE::update_ext_map()
{
    ...
    float3 loc_map_ur  = make_float3(loc_map_ll.x + param.local_size_x,
                                     ...
                                     loc_map_ll.z + param.local_size_z); //shm: coordination of the up-right coner of local map
